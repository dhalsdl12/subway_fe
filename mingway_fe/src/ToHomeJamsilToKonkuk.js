...
function ToWorkJamsil() {
  const [data, setData] = useState([]);

  useEffect(() => {
    // 초기 데이터를 가져오기 위해 한 번 호출
    fetchData('http://127.0.0.1:8000/ming/subway/home/jamsil-konkuk');
  }, []);
          ...
          data.map((stationData, index) => (
            <div key={index} className="station-info">
              ...
              <div className="arrival-info">
                {stationData['Jamsil To Konkuk Station'].map((arrival, idx) => (
